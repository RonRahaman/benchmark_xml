program main
  ...
  if (len_trim(path_input) .eq. 0 .or. len_trim(mode) .eq. 0) then
    print *, '***Error: Must specify an input dir and mode as command-line args'
    stop
  endif
  ...
  case default
    print *, 'Error: must specify a valid mode: fox_dom, xml_fortran_new'
  end select


  open(unit=600, file=trim(outfile), status='unknown', position='append')
  write(600, '(a20, i20, f20.10)') trim(mode), fsize, time
  close(unit=600)
  ...
  contains
    ...
    end subroutine read_geometry_xml_fox
  ...
  subroutine read_geometry_xml_fortran_new()
    ...
    subroutine benchmark_xml_fortran_old(time)
      double precision, intent(out) :: time
      integer :: cstart, cend, crate
      call system_clock(cstart, crate)
      call read_geometry_xml_fortran_old()
      call system_clock(cend, crate)
      time = dble(cend-cstart)/crate 
      if (allocated(cells)) deallocate(cells)
      if (allocated(lattices)) deallocate(lattices)
    ...
    end subroutine benchmark_xml_fortran_old

  subroutine read_geometry_xml_fortran_old()

    use xml_data_geometry_t_old
  ...
  end subroutine read_geometry_xml_fortran_old
