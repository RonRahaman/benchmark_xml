...
case ('geometry')
  ...
  print *, "***calling read_xml_file_geometry (v0) with filesize = ", fsize
  call benchmark_geometry(time)
  print *, 'read_xml_file_geometry (v0) completed in (seconds):', time


  open(unit=600, file='geometry_'//trim(outfile), status='unknown', position='append')
  write(600, '(a20, i20, f20.10)') 'v0', fsize, time
  close(unit=600)
...
case ('tallies')
  ...
  print *, "***calling read_xml_file_tallies (v0) with filesize = ", fsize
  call benchmark_tallies(time)
  print *, 'read_xml_file_tallies (v0) completed in (seconds):', time
